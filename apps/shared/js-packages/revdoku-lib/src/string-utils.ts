import { IDocumentFile } from './common-types';
...
export function cleanFilename(filename: string, separator: string = '-'): string {
    return filename.replace(/[^a-zA-Z0-9]/g, separator).replace(/-+/g, separator);
  }
  
  export function getFileNameFromFiles(files: IDocumentFile[]): string {
    const names = files.map(f => f.document_file_revisions?.[0]?.name || '').filter(name => name).join(' ');
    return cleanFilename(truncateFilename(names || 'untitled'), '-');
  }
  
  export function getTitleFromFiles(files: IDocumentFile[]): string {
    const names = files.map(f => f.document_file_revisions?.[0]?.name || '').filter(name => name).join(' ');
    return cleanFilename(truncateFilename(names || 'untitled'), ' ');
  }
