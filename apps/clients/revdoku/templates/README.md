...
`app-safe-actions.json` contains starter schemas and safe-action manifests for
...
3. Store a private `revdoku.app.json` contract file describing the app purpose,
   data model, safe actions, publish mode, and rollback notes.
4. Apply the template with `bucket_app_database_setup`.
5. Publish live with `site_type: "app"` only when requested.
