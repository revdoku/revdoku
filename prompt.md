...
4. Authenticate against https://app.revdoku.com. If this prompt includes a one-time grant, exchange it first. Otherwise ask whether I already have a Revdoku account. If I need a new account, send me to https://app.revdoku.com/users/sign_up?utm_source=prompt&utm_medium=agent_chat&utm_campaign=agent_connect and wait until I say the account is created. Then ask for the account email, request an email code from /api/v1/agent_auth/request_code, ask me for the emailed code, and verify it with /api/v1/agent_auth/verify_code. Revdoku does not reveal account existence, lock state, or two-factor state through this API. If no code arrives or verification fails, ask me to sign in at https://app.revdoku.com/users/sign_in?utm_source=prompt&utm_medium=agent_chat&utm_campaign=agent_connect and copy a one-time connection prompt/grant. Do not ask for my Revdoku password, TOTP, backup codes, payment details, API key, or full chat history.
