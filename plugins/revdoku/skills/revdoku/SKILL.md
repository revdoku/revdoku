...
Revdoku stores files in private buckets and publishes them as static websites or
SPAs. It supports Public, Password, and Require Email access, analytics, visitor
activity, and built-in forms. It does not run arbitrary server code, per-bucket
databases, cron jobs, or a client-side AI proxy.
...
- **Files already in Revdoku or text generated by a cloud agent:** use the
  hosted MCP tools at `https://app.revdoku.com/mcp`.
...
Use ordered `slug_suggestions` only when the user wants to influence a first
publication's URL. Analytics and browser-side event tracking default on; change
...
The CLI prints a Connection ID for visual comparison with the Revdoku approval
screen. Tell the user to confirm the same ID in Revdoku; never ask them to paste
or repeat it. Credentials are stored in `~/.revdoku/credentials` and must not be
printed.

Common commands:

- `revdoku p [PATH]`: upload and publish live; defaults to `.`.
